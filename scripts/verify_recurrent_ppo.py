...
def test_buffer_sizes():
    """Test that buffer sizes are correctly calculated for vectorized envs."""
    logger.info("=" * 80)
    logger.info("TEST 1: Buffer Size Calculation")
    logger.info("=" * 80)
    ...
    expected_buffer_size = n_steps * num_envs
    actual_buffer_size = len(trainer.buffer)

    logger.info(f"  num_envs: {num_envs}")
    logger.info(f"  n_steps: {n_steps}")
    logger.info(f"  Expected buffer_size: {expected_buffer_size}")
    logger.info(f"  Actual buffer size after init: {actual_buffer_size}")

    assert trainer.num_envs == num_envs, f"num_envs mismatch: {trainer.num_envs} != {num_envs}"
    assert trainer.is_vectorized == True, "Should detect vectorized env"

    logger.info("✓ TEST 1 PASSED: Buffer sizes correct\n")
    env.close()
    return True


def test_state_storage():
    """Test that recurrent states are properly stored and retrieved."""
    logger.info("=" * 80)
    logger.info("TEST 2: State Storage and Retrieval")
    logger.info("=" * 80)
