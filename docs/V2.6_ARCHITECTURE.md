...
### Core Philosophy
1. **Separation of Concerns**: Environment adapters ↔ Brain ↔ Controller
2. **Universal Interface**: All environments → (pixels, scalars, goal)
...
│                                                             │
│  ┌──────────────┐      ┌──────────────┐      ┌──────────┐ │
│  │  Gym Adapter │      │ MineRL Adapt │      │ Synthetic│ │
│  │              │      │              │      │ Dataset  │ │
│  └──────┬───────┘      └──────┬───────┘      └────┬─────┘ │
│         │                     │                   │        │
│         └─────────────────────┴───────────────────┘        │
...
- **Features:**
  ...
  - Inventory/compass → scalar features
  - 19 discrete action combinations
  - Camera look smoothing

**Synthetic Dataset (`synthetic_dataset.py`)**  **(Coming Soon)**
...
scalars [B, scalar_dim]
  → MLP
  → scalar_features [B, hidden_dim]

# Goal pathway
goal [B, goal_dim]
  → Embedding
  → goal_features [B, hidden_dim]
```
...
**V2.6 Enhancements:**
- ✅ Write clamping prevents overflow
- ✅ Adaptive eviction thresholds
- ✅ Min-slot protection (never evict critical memories)
...
#### V2.6 Enhancements:
- ✅ Exception-safe action fallback
- ✅ Pressure-driven memory expansion
...
│    adapter.reset() → BrainInputs(pixels, scalars, goal)  │
└───────────────────┬──────────────────────────────────────┘
                    │
┌───────────────────▼──────────────────────────────────────┐
│ 3. Controller Step                                       │
│    controller.step(pixels, scalars, goal) → action_idx   │
│    - Lookup agent slot                                   │
│    - Load hidden states                                  │
...
┌───────────────────▼──────────────────────────────────────┐
│ 4. Adapter Execution                                     │
│    adapter.step(action_idx) → (obs, reward, done, info)  │
└───────────────────┬──────────────────────────────────────┘
                    │
┌───────────────────▼──────────────────────────────────────┐
│ 5. Training Update (PPO)                                 │
│    - Collect rollout (N steps)                           │
│    - Compute advantages (GAE)                            │
│    - Update policy (clip objective)                      │
...
└─────────────┬──────────────────────┘
              │ KL Divergence Loss
┌─────────────▼──────────────────────┐
│ Student (BrainCyborgMind)          │
│ - Learns to mimic teacher          │
│ - Compresses knowledge             │
│ - Adds FRNN/LSTM/Memory            │
└────────────────────────────────────┘
```
