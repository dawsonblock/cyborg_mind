...
class PredictiveMemoryModule(nn.Module):
    ...
    def write(self, latent: torch.Tensor, memory: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Write to memory."""
        batch_size = latent.shape[0]
        ...
        erase = torch.sigmoid(self.erase_vectors(latent).view(batch_size, self.num_write_heads, self.memory_dim))
