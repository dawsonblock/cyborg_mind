...
class CyborgServer:
    def __init__(
        self, config_path: str = "config.yaml", checkpoint_path: str = "checkpoints/final_policy.pt"
    ):
        self.config = Config()
        ...
        self.device = get_device(
            "cpu"
        )  # Inference usually on CPU for low latency unless batch is huge
    ...
    def _setup_routes(self):
        ...
        @self.app.post(
            "/step", response_model=StepResponse, dependencies=[Depends(self._verify_token)]
        )
        async def step(req: StepRequest):
            ...
            try:
                ...
                obs_tensor = torch.tensor(
                    req.observation, device=self.device, dtype=torch.float32
                ).unsqueeze(0)
